...
io.on('connection', function (socket) {
  ...
  socket.on('verifypin', function (data) {
    ...
    if (!room) {
      console.log('verifypin: Attempting to join nonexistent room.');
      socket.emit('verifyerror', 'This room is no longer available.');
      return;
    } else {
      ...
      socket.emit('pinerror', 'Your PIN does not match this room.');
    }
  });
    ...
    if (processedAudio != null) {
      for (var member in availableRooms[roomId]['members']) {
        ...
        if (details['role'] == Role.LISTENER && details['isActive']) {
          ...
          io.to(details['socket']).binary(true).emit('playaudio', processedAudio);
        }
      }
    }
  });
  ...
  socket.on('endsession', async function (data) {
    ...
    var numberOfSamples = availableRooms[roomId]['audioProcessor'].sessionAudio.length;
    ...
    var file = fs.createReadStream(mp3FileName);
    ...
    // Submit composition mp3 along with its metadata
