...
io.on('connection', function (socket) {
    ...
    socket.on('leaveroom', function (roomId) {
        ...
        if (!memberList) {
            io.to(roomId).emit('roomerror', 'This room\'s member data is missing. Please exit.')
            return
        }
        ...
        if (user['isHost']) {
            ...
            io.of('/').in(roomId).clients((error, socketIds) => {
                ...
                socketIds.forEach(socketId => {
                    io.sockets.sockets[socketId].emit('roomerror', `The host has disconnected. Please exit the room.`)
        ...
        // If non-host is leaving, handle them exclusively
            ...
            if (!member) {
                console.log(`(leaveroom) Leaving member\'s data does not exist in room: ${roomId}.`)
                return
            }
    ...
    socket.on('joinroom', function (data) {
        ...
        if (!roomMembers) {
            ...
            socket.emit('joinerror', 'This room\'s member data seems to be missing. Please exit.')
            return
        }
        else {
            if (member['role'] == Role.LISTENER) {
                if (availableRooms[roomId]['currentListeners'] ==
                    availableRooms[roomId]['maxListeners']) {
                    socket.emit('roomerror', 'This room\'s max listener capacity was reached. Please exit.')
                    return
                }
                availableRooms[roomId]['currentListeners']++
            }
            else {
                if (availableRooms[roomId]['currentPerformers'] ==
                    availableRooms[roomId]['maxPerformers']) {
                    console.log('Max performers reached...')
                    socket.emit('roomerror', 'This room\'s max performer capacity was reached. Please exit.')
                    return
                }
                availableRooms[roomId]['currentPerformers']++
            }
    ...
    socket.on('startsession', function (roomId) {
        ...
        if (!existingRoom) {
            io.to(roomId).emit('roomerror', 'Non-host is attempting to start session. Functionality is unavailable.')
            return
        }
    ...
    socket.on('endsession', function (roomId) {
        ...
        if (!existingRoom) {
            io.to(roomId).emit('roomerror', 'Cannot end session of nonexistent room. Please exit.')
            return
        }
